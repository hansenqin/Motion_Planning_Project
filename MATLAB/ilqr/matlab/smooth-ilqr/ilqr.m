    ...
    methods
        ...
        function [states,inputs,k_feedforward,K_feedback,current_cost] = solve(self)
            ...
            for ii = 1:self.n_iterations_
                ...
                while(learning_rate > 0.05 && armijo_flag == 0)
                    ...
                    new_inputs_steer = new_inputs(:,2);
                    
                    new_inputs_steer(abs(new_inputs_steer)>pi/3) = sign(new_inputs_steer(abs(new_inputs_steer)>pi/3))*pi/3;
                    
                    new_inputs(:,2) = new_inputs_steer;
        ...
        function total_cost = compute_cost(self,states,inputs)
            ...
            for ii = 2:self.n_timesteps_
                ...
                R_diff(1,1) = 0.1;
                R_diff(2,2) = 100;
                diff_cost = (current_u'-inputs(ii-1,:))./self.dt_*R_diff*(current_u'-inputs(ii-1,:))'./self.dt_;
                state_cost = ((self.target_state_-self.init_state_)-states(ii,:)')'*self.Q_k_*(self.target_state_-self.init_state_-states(ii,:)');
                current_cost = current_u'*self.R_k_*current_u; % Right now only considering cost in input
                total_cost = total_cost+current_cost+diff_cost;%+state_cost;
            end
