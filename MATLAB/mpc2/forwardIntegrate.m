...
% Created: 31 Oct 2021
    ...
    while t_total < TOTAL_TIME 
        curr_pos = Y(round((iteration-1)*dt/0.01+1,0) , [1,3] ); % record current vehicle position
        Xobs_seen = senseObstacles(curr_pos, Xobs); % sense the obstacles within 150m
        curr_state = Y(round((iteration-1)*dt/0.01+1,0) , : ); % record current vehicle states
        if iteration ==1
            curr_state = [283     5  -176     0     2.1     0];
        end
        %debug plot
        scatter(curr_pos(1), curr_pos(2),'.')
        xlim([curr_pos(1)-100, curr_pos(1)+100])
        ylim([curr_pos(2)-100, curr_pos(2)+100])
        pause(0.0001)
        ...
        [Utemp, FLAG_terminate] = ROB535_ControlProject_part2_Team01(TestTrack,Xobs_seen,curr_state); %%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
